...
tags: ["query:tr-hb"]
score: 7.0
evidence: 代谢率和能量分配对温度和资源的响应
tldr: 该研究探讨了原生动物如何通过调整能量分配来应对温度和资源变化。研究者建立了一个基于生物物理原理的预测框架，并通过对梨形四膜虫的实验发现，虽然短期内代谢和运动速度随温度剧增，但长期（约3.5天）通过改变细胞大小等表型调节，生物能恢复更高效的能量分配状态，揭示了体型在微生物适应环境中的核心作用。
source: biorxiv
selection_source: fresh_fetch
motivation: 旨在理解多种表型特征如何相互作用并与环境条件结合，共同塑造生物体的能量分配策略。
method: 建立基于能量和生物物理原理的预测框架，并对梨形四膜虫在不同温度和资源组合下的生长、体型、呼吸和运动进行实验测量。
result: 急性温度升高导致呼吸和运动速度增加，但约3.5天后，通过细胞大小的改变，各项表型特征趋于恢复至更高效的能量分配水平。
conclusion: 细胞大小的快速变化是微生物应对环境波动的关键机制，能有效调节代谢率和运动能耗以实现能量平衡。
---

## 摘要
为了生存和繁衍，生物体需要在能量摄入和能量消耗之间保持有效的平衡。环境条件的变化可能会破坏先前有效的能量分配策略，要求生物体改变其行为、生理或形态以应对新环境。然而，多种表型特征如何相互作用以及如何与环境条件相互作用以塑造能量分配，目前仍知之甚少。为了更好地理解这种表型-环境相互作用，我们建立了一个基于能量学和生物物理学原理的预测框架，该框架允许我们预测代谢率和运动速度应如何响应环境温度和资源供应而变化，并区分了对新条件的短期急性暴露和允许驯化或适应的长期暴露。我们通过将嗜热四膜虫（Tetrahymena pyriformis）的无菌种群暴露于温度和资源可用性的不同组合中来测试这些预测。我们测量了种群增长、细胞大小、呼吸作用和运动。温度的急性升高导致了更高的运动速度和呼吸速率，这与玻尔兹曼-阿伦尼乌斯方程（Boltzmann-Arrhenius equation）以及作用于运动的粘性阻力等物理标度关系的预期一致。然而，在将四膜虫引入新环境约 3.5 天后，所有测量的特征都向更接近原始环境的值转变。这些变化可能反映了表型驯化反应，在新条件下恢复了更有效的能量分配。细胞大小的变化在这一过程中起到了关键作用，它同时影响了包括代谢率和运动能量消耗在内的多种表型特征。在像四膜虫这样的小型微生物消费者中，相对于生态和季节时间尺度，体型可以迅速发生变化。因此，体型的变化可以与生理和生化调节一起被有效地利用，以应对环境变化。
