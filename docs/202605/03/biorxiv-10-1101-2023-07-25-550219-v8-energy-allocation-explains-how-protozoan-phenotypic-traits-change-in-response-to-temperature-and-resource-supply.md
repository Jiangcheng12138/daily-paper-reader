...
title_zh: 能量分配解释了原生动物表型特征如何响应温度和资源供应而变化
authors: "Perna, A., Rivoli, E. S., Reiss, J., Perkins, D. M."
date: 2026-04-27
pdf: "https://www.biorxiv.org/content/10.1101/2023.07.25.550219v8.full.pdf"
tags: ["query:tr-hb"]
score: 6.0
evidence: 响应温度变化的代谢率与能量分配
tldr: 本研究探讨了原生动物如何通过调整能量分配来应对温度和资源变化。研究者基于能量学和生物物理原理开发了一个预测框架，并以梨形四膜虫为对象，实验测量了其在不同环境下的生长、体型、呼吸和运动。研究发现，虽然短期高温会显著提高代谢和运动速度，但长期暴露后，生物会通过改变体型等表型调节实现能量分配的重新平衡。这一发现揭示了体型在微生物应对环境变化中的核心作用。
source: biorxiv
selection_source: fresh_fetch
motivation: 旨在理解生物如何通过多重表型性状的交互作用来调整能量分配，以应对环境温度和资源供应的变化。
method: 基于生物物理原理构建预测框架，并通过实验测量梨形四膜虫在不同温度和资源组合下的代谢、运动及体型变化。
result: 急性升温虽导致代谢和运动速度增加，但经过约3.5天的驯化后，各项性状会通过体型缩小等机制趋向于恢复初始状态。
conclusion: 体型变化是微生物在生理调节之外，应对环境压力并优化能量分配的关键表型策略。
---

## 摘要
为了生存和繁衍，生物体需要维持能量摄入与能量消耗之间的高效平衡。环境条件的改变可能会破坏先前高效的能量分配策略，生物体需要改变其行为、生理或形态以应对新环境。然而，多种表型特征如何相互作用以及如何与环境条件相互作用以塑造能量分配，目前仍知之甚少。为了更好地理解这种表型-环境相互作用，我们建立了一个基于能量学和生物物理学原理的预测框架，该框架允许我们预测代谢率和运动速度应如何响应环境温度和资源供应而变化，并区分了对新条件的短期急性暴露与允许驯化或适应的长期暴露。我们通过将嗜热四膜虫（Tetrahymena pyriformis）的无菌种群暴露于温度和资源可用性的不同组合中来测试这些预测。我们测量了种群增长、细胞大小、呼吸作用和运动。温度的急性升高导致了更高的运动速度和呼吸速率，这与玻尔兹曼-阿伦尼乌斯方程（Boltzmann-Arrhenius equation）以及作用于运动的粘性阻力等物理定标关系的预期一致。然而，在将四膜虫引入新环境约 3.5 天后，所有测量的特征都向更接近原始环境的值转变。这些变化可能反映了表型驯化反应，在新条件下恢复了更高效的能量分配。细胞大小的变化在这一过程中起到了关键作用，它同时影响了包括代谢率和运动能量消耗在内的多种表型特征。在像四膜虫这样的小型微生物消费者中，体型相对于生态和季节时间尺度可以发生快速变化。因此，体型的变化可以与生理和生化调节一起被有效地利用，以应对环境变化。
