...
tags: ["query:qll"]
score: 7.0
evidence: 跨组织和寿命的衰老细胞多模态数据集
tldr: 细胞衰老是衰老的核心特征，但其异质性给研究带来挑战。SenNet计划通过建立SenNet数据门户，整合了人类和老鼠多组织、多模态的单细胞、空间及组学数据。该门户采用混合云微服务架构，提供标准化协议、生物标志物目录及分析工具，旨在为全球研究者提供全面识别和表征衰老细胞的资源，推动衰老机制研究与生物标志物发现。
source: biorxiv
selection_source: fresh_fetch
motivation: 针对细胞衰老的异质性和背景依赖性，建立一个统一的公共平台以整合多模态、多组织的衰老研究数据。
method: 构建基于混合云微服务架构的SenNet门户，集成数据管理、标准化处理流程、空间背景映射及跨物种分析工具。
result: 截至2026年4月，门户已托管涵盖15个器官、6种检测类型的2041个公开数据集，并提供丰富的生物标志物和实验协议。
conclusion: SenNet门户为衰老研究提供了关键的基础设施，通过开放共享的数据和工具，显著促进了衰老细胞的表征、发现及跨学科协作。
---

## 摘要
细胞衰老是衰老的一个标志，也是各组织功能衰退的驱动因素，但其异质性和背景依赖性限制了系统性研究。美国国立卫生研究院公共基金（Common Fund）的细胞衰老网络（SenNet）计划通过生成多模态、多组织数据集来应对这一挑战，这些数据集描绘了人类整个生命周期及互补小鼠模型中的衰老细胞特征。SenNet 数据门户（https://data.sennetconsortium.org）作为这些资源的公共门户，提供了对统一的单细胞、空间、成像、转录组学和蛋白质组学数据的开放访问；此外还包括衰老生物标志物目录以及可用于全面识别和表征小鼠及人类组织中衰老细胞的标准方案。截至 2026 年 4 月，该门户托管了涵盖 15 个器官、使用 6 种通用分析类型的 2,041 个公开的人类和小鼠数据集。来自 13 个组织图谱中心（TMC）和 12 个技术开发与应用（TDA）组件的专家贡献了组织数据、分析数据、识别衰老生物标志物，并就跨组织抗体统一的面板达成一致。他们还将人类组织数据注册到人类参考图谱（HRA）中，并开发了用于对这些数据进行多尺度和多模态探索的用户界面。该门户由联盟组织和数据协调中心（CODCC）基于可扩展的混合云微服务架构构建，实现了数据提交、管理、集成分析、空间背景映射以及对衰老研究至关重要的跨物种数据的统一访问。本文介绍了用户需求、门户架构、数据处理工作流以及专注于衰老的分析工具；展示了在生物标志物发现、质量基准测试、假设生成、空间分析、高性价比分析和细胞距离分布分析中的应用场景；以及广大研究社区的效用和使用情况。文中还讨论了当前的局限性和计划中的扩展，包括扩展的空间组学发布和改进的衰老表型（senotype）表征工具。SenNet 方案、代码和用户界面可在 https://docs.sennetconsortium.org/apis 免费获取。
