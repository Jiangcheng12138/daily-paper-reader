...
tags: ["query:slp-ns"]
score: 7.0
evidence: 针对大规模神经科学数据集的综合神经控制论建模
tldr: 针对当前大规模神经科学数据建模碎片化的问题，本文提出了一种集成神经控制论建模框架。该方法主张将大脑视为追求潜在目标的控制器，通过整合非线性状态空间模型、元动力学扩展及连接组学架构，捕捉大脑、身体与环境的闭环耦合。这种模型旨在从异构数据中推断神经与行为动力学的组织原则，为理解大脑如何产生行为提供了一种从数据驱动转向机制驱动的系统性路径。
source: arxiv
...
motivation: 现有的神经科学建模工作往往局限于孤立的实验，缺乏对大脑、身体与环境之间闭环交互的整合性理解。
method: 结合非线性状态空间模型、元动力学扩展、可扩展推理及连接组学启发式架构，构建能够处理异构数据的集成模型。
result: 该框架通过整合多源实验约束，实现了统计增强、少样本泛化以及对神经动力学和控制目标的深刻机制洞察。
conclusion: 集成神经控制论模型为从碎片化数据转向研究大脑产生行为的机制科学提供了一条以模型为中心的切实路径。
---

## 摘要
大规模神经科学正在产生跨动物、脑区和行为背景的丰富数据集，然而我们的建模工作在孤立的实验中仍然是碎片化的。我们认为，理解行为需要整合性神经控制论模型：即能够捕捉大脑、身体与环境闭环耦合的可理解动力学模型，将大脑视为追求潜在目标的控制器，表征跨尺度的结构化变异，并能扩展到异构数据集。此类模型将目标从孤立地预测神经记录转向推断支配神经和行为动力学的组织原则。我们通过结合非线性状态空间模型和元动力学扩展，以及可扩展推理、知识蒸馏、混合开闭环训练和受连接组学启发的架构，概述了实现这一目标的实际路径。通过汇集来自记录、行为、扰动和解剖结构的互补约束，整合性神经控制论模型可以提供统计放大、少样本泛化，以及对共享动力学结构、个体差异和支配行为的控制目标的机制性见解。这一议程提供了一条以模型为中心的路径，从碎片化数据走向关于大脑如何产生行为的机制科学。
