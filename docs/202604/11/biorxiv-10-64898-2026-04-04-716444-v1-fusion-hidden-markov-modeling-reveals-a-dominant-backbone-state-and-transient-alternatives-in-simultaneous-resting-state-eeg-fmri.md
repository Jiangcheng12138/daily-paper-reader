...
title_zh: 融合隐马尔可夫建模揭示同步静息态 EEG-fMRI 中的主导骨干状态与瞬态替代状态
authors: "Cruz, G. E."
date: 2026-04-07
...
tldr: 本研究针对同步EEG-fMRI多模态融合建模的挑战，开发了一套基于隐马尔可夫模型（HMM）的融合框架。通过对12名健康受试者的静息态数据进行分析，研究发现大脑动态由一个占主导地位的“骨干状态”和两个瞬态替代状态组成。该方法不仅提供了实用的全脑融合工作流，还揭示了电生理与血氧动力学网络在不同潜在状态下的差异化表达，为理解静息态脑动态提供了新的生物学视角。
source: biorxiv
...
method: 开发了一种基于隐马尔可夫模型（HMM）的融合框架，通过多模态对齐和留一法交叉验证确定了最优的三状态模型。
result: 发现大脑存在一个高占用率且持久的“骨干状态”，以及两个分别表现为信号衰减和选择性网络重构的瞬态替代状态。
conclusion: 该研究为同步EEG-fMRI融合提供了实用的全脑分析流程，并证明了电生理与血氧动力学网络在不同潜在脑状态下具有不同的关联结构。
---

## 摘要
同步 EEG-fMRI 为研究大脑动力学提供了一种强大的手段，但将这两种模态整合到一个通用的全脑模型中仍然具有挑战性。在本研究中，我开发了一个用于静息态同步 EEG-fMRI 的融合建模框架，该框架强调精细的多模态对齐、稳定共享特征空间的构建，以及基于交叉验证和重现性的模型选择。利用来自公开同步 EEG-fMRI 数据集的 12 名健康成人的 15 次睁眼静息态运行数据，我构建了一个无滞后、最小 15 TR 的融合数据集，包含 3550 个保留的 TR 和 124.25 分钟的可用数据。留一受试者交叉验证（leave-one-subject-out cross-validation）支持一个简洁的三状态融合隐马尔可夫模型。在最终的全数据解中，一个状态表现为主导骨干状态，具有最高的占用率、最强的持续性以及最清晰的标准 BOLD 网络组织。两个较低占用率的状态表现为瞬态替代状态：其中一个表现为骨干状态的广泛减弱版本，而另一个则显示出更具选择性的网络权重重组。这些状态在描述性的跨模态 BOLD-EEG 结构上也存在差异，表明电生理和血流动力学网络表达在不同的潜在脑状态下可能具有不同的对齐方式。这些结果既提供了一个实用的全脑 EEG-fMRI 融合工作流，也对低阶静息态大脑动力学给出了具有生物学解释性的说明。
