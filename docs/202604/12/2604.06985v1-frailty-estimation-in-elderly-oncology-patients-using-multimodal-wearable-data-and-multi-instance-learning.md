...
tldr: 本研究针对老年癌症患者衰弱评估频率不足的问题，提出一种基于多模态可穿戴数据的监测框架。通过融合智能手表的活动与睡眠数据以及胸带心率变异性（HRV）特征，利用注意力机制多示例学习（MIL）模型处理现实中数据缺失和不规则采样问题，实现了对患者功能状态变化（如手握力和疲劳度）的有效预测，为临床提供了诊室外的持续评估手段。
source: arxiv
...
motivation: 旨在解决老年肿瘤患者衰弱评估仅限于低频次门诊随访、无法实时捕捉功能衰退的问题。
method: 采用基于注意力机制的多示例学习框架，融合多模态可穿戴数据以应对真实场景下的数据缺失和弱监督挑战。
result: 模型在预测手握力和疲劳量表变化方面表现良好，6个月随访时的手握力预测平衡准确率达到0.70。
conclusion: 智能手表监测的活动与睡眠是预测衰弱变化的关键指标，而HRV特征在多模态融合中起到了重要的补充作用。
---
