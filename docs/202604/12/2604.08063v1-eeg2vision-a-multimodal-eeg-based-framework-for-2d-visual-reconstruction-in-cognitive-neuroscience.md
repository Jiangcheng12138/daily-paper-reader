...
tldr: 本研究提出EEG2Vision框架，旨在解决非侵入式EEG在视觉重建中空间分辨率低和噪声大的挑战。该框架采用模块化设计，通过EEG条件扩散模型实现初步重建，并引入基于多模态大语言模型的后期增强机制，利用语义提示优化图像的几何结构和感知一致性。研究系统评估了不同通道密度（128至24通道）下的表现，证明了该方法在低分辨率设备上依然能保持较高的重建质量，显著提升了脑机接口应用的实用性。
source: arxiv
