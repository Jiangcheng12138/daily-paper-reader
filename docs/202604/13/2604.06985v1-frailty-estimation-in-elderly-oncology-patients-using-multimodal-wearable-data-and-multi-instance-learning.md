...
tags: ["query:slp-ns"]
score: 6.0
evidence: 包含睡眠特征的穿戴式数据用于老年人衰弱评估
tldr: 本研究针对老年癌症患者衰弱评估频率低的问题，提出一种基于多模态可穿戴数据的监测框架。通过融合智能手表的体力活动、睡眠数据以及胸带的HRV特征，利用注意力机制的多示例学习（MIL）模型处理现实中数据缺失和弱监督问题，实现了对乳腺癌患者随访期间功能状态变化（如握力和疲劳度）的有效预测。
source: arxiv
...
motivation: 旨在解决老年肿瘤患者在两次诊室随访之间缺乏持续衰弱评估和功能衰退监测的问题。
method: 采用基于注意力机制的多示例学习框架，结合多层感知器编码器，融合不规则且存在缺失的多模态纵向可穿戴数据。
result: 模型在预测握力变化方面达到最高0.70的平衡准确率，在预测疲劳量表变化方面达到0.64，表现出良好的预测性能。
conclusion: 智能手表的活动与睡眠数据是预测衰弱变化的关键，而HRV特征在多模态融合中提供了重要的补充信息。
---
