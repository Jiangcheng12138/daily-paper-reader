...
## 摘要
为了生存和繁衍，生物体需要在能量摄入和能量消耗之间维持有效的平衡。环境条件的变化会破坏先前有效的能量分配策略，使生物体必须改变其行为、生理或形态以应对新环境。然而，多种表型特征如何相互作用以及如何与环境条件相互作用以塑造能量分配，目前仍知之甚少。为了更好地理解这类表型-环境相互作用，我们开发了一个基于能量和生物物理原理的预测框架，该框架允许我们预测代谢率和运动速度应如何响应环境温度和资源供应而变化，并区分了对新条件的短期急性暴露与允许驯化或适应的长期暴露。我们通过将嗜热四膜虫（Tetrahymena pyriformis）的无菌种群暴露于温度和资源可用性的不同组合中来测试这些预测。我们测量了种群增长、细胞大小、呼吸作用和运动。温度的急性升高导致了更高的运动速度和呼吸速率，这与玻尔兹曼-阿伦尼乌斯方程（Boltzmann-Arrhenius equation）以及作用于运动的粘性阻力等物理标度关系的预期一致。然而，在将四膜虫引入新环境约3.5天后，所有测量的特征都向接近原始环境的值转变。这些变化可能反映了表型驯化反应，在新条件下恢复了更有效的能量分配。细胞大小的变化在这一过程中起到了关键作用，它同时影响了包括代谢率和运动能量成本在内的多种表型特征。在像四膜虫这样的小型微生物消费者中，体型相对于生态和季节时间尺度可以发生快速变化。因此，体型变化可以与生理和生化调节手段共同被有效地利用，以应对环境变化。
